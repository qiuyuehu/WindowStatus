...
## 阶段：P1 - 弹窗样式统一（oklch 配色 + 通用组件）

**目标**：提取通用样式模块，统一三个弹窗的配色和组件
...
2. 语法检查通过：`python3 -m py_compile plugins/common_styles.py plugins/stats/dialog.py plugins/settings/dialog.py plugins/about/plugin.py`
3. 弹窗样式统一（oklch 配色 + 通用组件）
4. 手动测试：打开统计、设置、关于弹窗，确认样式一致
