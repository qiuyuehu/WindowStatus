...
class SettingsPlugin(Plugin):
    ...
    def _get_plugins_info(self) -> List[dict]:
        """获取所有插件的信息"""
        plugins_info = []
        for plugin in self.kernel.plugin_manager.get_all_plugins():
            plugins_info.append({
                "name": plugin.name,
    ...
    def _on_show_settings(self, **kwargs):
        ...
        try:
            ...
            dialog = SettingsDialog(categories, plugins_info, current_position, current_theme)
    ...
    def _on_save(self, result: dict):
        ...
        Args:
            result: {"categories": {...}, "plugins": {...} or None, "position": "...", "theme": "..."}
        """
        try:
            with self.config.batch_update():
                ...
                if categories:
                    ...
                    # 通知 Rules 插件重新加载
                    rules_plugin = self.kernel.plugin_manager.get_plugin("rules")
                    if rules_plugin and hasattr(rules_plugin, "reload_rules"):
                        rules_plugin.reload_rules()
                ...
                if theme:
                    ...
                    if overlay_plugin and overlay_plugin.widget:
                        overlay_plugin.widget.theme = theme
