...
### 新功能
- **桌宠记住位置**：拖拽桌宠后自动保存位置，重启时恢复桌宠和气泡到上次位置（TDD 方式实现）
- **统计界面重做**：Apple Screen Time 风格，摘要卡片 + 环形占比图 + 彩色进度条 + 导出按钮
- **安装 tdd skill**：测试驱动开发，先写测试再写代码
- **grill-me 增强**：融合 grill-with-docs 理念（追问时读代码验证、决策沉淀、术语一致性）

### 优化
- 统计弹窗窗口从 600x500 扩大到 750x600
- 周/月统计 tab 加入环形图和摘要区
- 时间线 tab 加入摘要区（记录数 + 总时长）
- 分类图标空值显示 ❓
- 4 个 tab 布局统一（80px 摘要区 + 下方内容）
